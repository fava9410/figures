...
class TestLearnerCourseGradeMetricsManager(object):
    ...
    def test_completed_for_site_empty(self):
        """Make sure the method doesn't break if there are no LCGM records
        """
        data = LearnerCourseGradeMetrics.objects.completed_for_site(self.site)
        assert isinstance(data, QuerySet)
        assert not data

    def test_completed_for_site(self):
        """
        Example data
        ```
    ...
    def test_completed_for_user_list(self):
    ...
    def test_completed_for_course_list(self):
    ...
    def test_completed_for_user_and_course_lists(self):
        """Test when we filter on a set of course_ids
        """
        lcgm_data = create_sample_completed_lcgm(self.site,
                                                 user_count=4,
